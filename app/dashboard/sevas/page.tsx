...
export default function SevasPage() {
  ...
  const [form, setForm] = useState({ name: "", description: "", amount: "0", centerId: "" });
  ...
  function startEdit(s: SevaRecord) { setEditing(s); setForm({ name: s.name, description: s.description, amount: String(s.amount), centerId: s.centerId }); setShowForm(true); }
  function startNew() { setEditing(null); setForm({ name: "", description: "", amount: "0", centerId: "" }); setShowForm(true); }

  async function handleSave() {
    const amount = Number(form.amount) || 0;
    if (editing) {
      await callApi("update", { record: { type: "seva", __backendId: editing.id, name: form.name, description: form.description, amount, centerId: form.centerId || undefined } });
    } else {
      await callApi("create", { record: { type: "seva", name: form.name, description: form.description, amount, centerId: form.centerId || undefined } });
    }
